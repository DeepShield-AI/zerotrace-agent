...
use crate::{
    collector::{
        Collector, CollectorThread, L7Collector, L7CollectorThread, MetricsType,
        flow_aggr::FlowAggrThread, l7_quadruple_generator::L7QuadrupleGeneratorThread,
        quadruple_generator::QuadrupleGeneratorThread,
    },
    common::{
        DEFAULT_LOG_RETENTION, DEFAULT_LOG_UNCOMPRESSED_FILE_COUNT, DEFAULT_TRIDENT_CONF_FILE,
        FREE_SPACE_REQUIREMENT, FeatureFlags,
    ...
    config::{
        Config, ConfigError, DpdkSource, PcapStream, UserConfig,
        handler::{ConfigHandler, DispatcherConfig, ModuleConfig},
    },
    debug::{ConstructDebugCtx, Debugger},
    dispatcher::{
        self, BpfOptions, Dispatcher, DispatcherBuilder, DispatcherListener, recv_engine::bpf,
    },
    exception::ExceptionHandler,
    flow_generator::{
        PacketSequenceParser, TIME_UNIT,
        protocol_logs::{BoxAppProtoLogsData, SessionAggregator},
    },
    ...
    rpc::{DEFAULT_TIMEOUT, Session, Synchronizer},
    sender::{
        npb_sender::NpbArpTable,
        uniform_sender::{Connection, UniformSenderThread},
    },
    utils::{
        cgroups::{Cgroups, is_kernel_available_for_cgroups},
...
use enterprise_utils::kernel_version::{ActionFlags, kernel_version_check};
use flexi_logger::{
    Age, Cleanup, Criterion, FileSpec, Logger, Naming, colored_opt_format, writers::LogWriter,
};
...
use public::{
    ...
    utils::net::{IpMacPair, Link, MacAddr, get_route_src_ip},
};
#[cfg(target_os = "linux")]
...
impl AgentState {
    ...
    pub fn enable(&self) {
        ...
        if old_state != new_state {
            info!(
                "Agent state changed from {old_state:?} to {new_state:?} (enabled: {} melted_down: {})",
                sg.0.enabled, sg.0.melted_down
            );
            self.notifier.notify_one();
        }
    }
    ...
    pub fn start<P: AsRef<Path>>(
        ...
        let main_loop = thread::Builder::new().name("main-loop".to_owned()).spawn(move || {
            if let Err(e) = Self::run(
                ...
                error!(
                    "Launching zerotrace-agent failed: {}, zerotrace-agent restart...",
                    e
                );
    ...
    fn run(
        ...
        let agent_id = if sidecar_mode {
            ...
            if let Err(e) = netns::NsFile::Root.open_and_setns() {
                return Err(anyhow!(
                    "agent must have CAP_SYS_ADMIN to run without 'hostNetwork: true'. setns error: {}",
                    e
                ));
            }
        ...
        if running_in_container() {
            ...
            info!(
                "don't initialize cgroups controller, because kernel version < 3 or agent is in Windows"
            );
        } else if cgroups_disabled {
            // 如果配置显式禁用 Cgroups，则退化为定期轮询检查资源使用情况。
            info!(
                "don't initialize cgroups controller, disable cgroups, zerotrace-agent will default to checking the CPU and memory resource usage in a loop every 10 seconds to prevent resource usage from exceeding limits"
            );
        } else {
            ...
            match Cgroups::new(process::id() as u64, config_handler.environment()) {
                ...
                Err(e) => {
                    warn!(
                        "initialize cgroups controller failed: {}, resource utilization will be checked regularly to prevent resource usage from exceeding the limit.",
                        e
                    );
...
impl WatcherComponents {
    ...
    fn new(
        ...
        info!(
            "This agent will only watch K8s resource because IN_CONTAINER={} and K8S_WATCH_POLICY={}",
            env::var(IN_CONTAINER).unwrap_or_default(),
            env::var(K8S_WATCH_POLICY).unwrap_or_default()
        );
...
impl AgentComponents {
    ...
    fn new(
        ...
        {
            warn!(
                "src_interfaces is not empty, but this has already been deprecated, instead, the tap_interface_regex should be set"
            );
        }
        ...
        if candidate_config.dispatcher.extra_netns_regex != "" {
            ...
            if candidate_config.capture_mode == PacketCaptureType::Local {
                ...
                log::error!(
                    "When the PacketCaptureType is not Local, it does not support extra_netns_regex, other modes only support interfaces under the root network namespace"
                );
            }
        }
...
fn build_dispatchers(
    ...
    let dispatcher = match dispatcher_builder.build() {
        ...
        Err(e) => {
            warn!(
                "dispatcher creation failed: {}, zerotrace-agent restart...",
                e
            );
