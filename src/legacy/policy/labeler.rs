...
impl Labeler {
    ...
    // 注意当查询外网时必须给epc参数传递EPC_ZEROTRACE值，表示在所有WAN CIDR范围内搜索，并返回该CIDR的真实EPC
            ...
            if TunnelInfo::is_gre_pseudo_inner_mac(mac) {
                ...
                if info.l3_epc_id == 0 {
                    ...
                    if let Some(interface) = self.get_interface_by_ip(ip) {
                        ...
                        is_wan = self.set_epc_by_cidr(ip, EPC_ZEROTRACE, &mut info);
                    }
                    return (info, is_wan);
                }
            } else {
    ...
    fn get_l3_by_wan(&self, src: IpAddr, dst: IpAddr, endpoint: &mut EndpointData) -> (bool, bool) {
        ...
        if src_data.l3_epc_id == 0 {
            ...
            if let Some(interface) = self.get_interface_by_ip(src) {
                ...
                found_src = self.set_epc_by_cidr(src, EPC_ZEROTRACE, src_data);
            }
        }
        if dst_data.l3_epc_id == 0 {
            ...
            if let Some(interface) = self.get_interface_by_ip(dst) {
                ...
                // step 2: 查询ZEROTRACE添加的WAN监控网段(cidr)
                fount_dst = self.set_epc_by_cidr(dst, EPC_ZEROTRACE, dst_data);
            }
        }
    ...
    fn get_vip(
        ...
        if !src_data.is_vip && src_data.l3_epc_id > 0 {
            if !is_src_wan {
                ...
                self.set_vip_by_cidr(key.src_ip, EPC_ZEROTRACE, &mut src_data);
            }
        }
        if src_data.is_vip {
            src_data.real_ip =
        ...
        if !dst_data.is_vip && dst_data.l3_epc_id > 0 {
            if !is_dst_wan {
                ...
                self.set_vip_by_cidr(key.dst_ip, EPC_ZEROTRACE, &mut dst_data);
            }
        }
        if dst_data.is_vip {
            dst_data.real_ip =
...
mod tests {
    ...
    fn test_epc_ip_normal() {
        ...
        let ret = labeler.get_interface_by_epc_ip("192.168.10.200".parse().unwrap(), EPC_ZEROTRACE);
