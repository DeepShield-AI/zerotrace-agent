	...
	for rule in rules {
		...
		for route in routes.iter() {
			...
			if rule.table == route.table && route_ip.contains(ip) {
				trace!("found prefix len {} for addr {}", route_ip.prefix_len(), ip.addr());
...
pub fn interfaces_linked_with<S: AsRef<[NsFile]>>(
	...
	debug!("query namespaces: {:?}", all_ns.iter().map(|(k, v)| (k, &v[0])).collect::<Vec<_>>());
		...
		.filter_map(|f| match open_root_or_named_ns_file(f) {
			Ok((fp, _)) => {
				if let Err(_) = set_netns(&fp) {
					...
					Some(NamespaceInfo { fp, file: f, prefix_map })
				}
			},
			Err(e) => {
	...
	'outer: for (ino, paths) in all_ns.iter() {
		...
		for path in paths {
			...
			nsid_map.clear();
			for NamespaceInfo { file: ns, fp: nsfp, prefix_map } in interested_files.iter() {
	...
	SUPPORTS_NETNS.with(|f| {
		*f.get_or_init(|| {
			if fs::metadata(SELF_NS_PATH).is_ok() {
				...
				info!("path {} is not accessible, setns() calls will be NOOP", SELF_NS_PATH);
				false
			}
		})
	})
...
pub fn set_netns(fp: &File) -> Result<()> {
	...
	if let Err(e) = setns(fp.as_raw_fd(), CloneFlags::CLONE_NEWNET) {
		debug!("setns({}) failed for inode {:?}: {:?}", fp.as_raw_fd(), target_inode, e);
		return Err(e.into());
	}
...
impl WrappedSocket {
	...
	fn get_nsid_by_file(&mut self, fp: &File) -> Result<i32> {
		...
		Rtgenmsg { rtgen_family: RtAddrFamily::Unspecified }.to_bytes(&mut payload)?;
		// padding
		payload.write(&[0, 0, 0])?;
