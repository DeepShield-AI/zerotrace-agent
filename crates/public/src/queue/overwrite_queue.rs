...
impl<T> OverwriteQueue<T> {
	...
	unsafe fn raw_send(&self, msgs: *const T, count: usize) -> Result<(), Error<T>> {
		...
		let end = if raw_end < start { raw_end + 2 * self.size } else { raw_end };
		...
		if end - start + count > self.size {
			...
			if free_space < count {
				...
				self.start
					.store((start + to_overwrite) & (2 * self.size - 1), Ordering::Release);
	...
	unsafe fn raw_recv_timeout(
		...
		self.start.store((start + recv_count) & (2 * self.size - 1), Ordering::Release);
...
impl<T> RefCounter<T> {
	pub fn new(queue: OverwriteQueue<T>) -> (Sender<T>, Receiver<T>, StatsHandle<T>) {
		...
		(Sender { counter }, Receiver { counter }, StatsHandle { counter })
	}
}
...
mod tests {
	...
	fn receive_multiple() {
...

		{
			...
			s.send_all(&mut vec![CountedU64::new(42, c.clone()), CountedU64::new(43, c.clone())])
				.unwrap();
			s.send(CountedU64::new(44, c.clone())).unwrap();
			...
			s.send_all(&mut vec![CountedU64::new(45, c.clone()), CountedU64::new(46, c.clone())])
				.unwrap();
