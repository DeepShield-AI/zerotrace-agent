...
pub fn get_mac_by_ip(ip: IpAddr) -> Result<MacAddr> {
	...
	let if_idx = addrs
		.iter()
		.find_map(|a| if a.ip_addr == ip { Some(a.if_index) } else { None })
		.ok_or(Error::LinkIdxNotFoundByIP(format!("can't find interface index by ip {}", ip)))?;
	// 找网卡，返回网卡mac地址
	let mac = links
		.iter()
		.find_map(|l| if l.if_index == if_idx { Some(l.mac_addr) } else { None })
		.ok_or(Error::LinkIdxNotFoundByIP(format!("can't find mac address by ip {}", ip)))?;
...
impl Default for IpMacPair {
	fn default() -> Self {
		Self { ip: IpAddr::V4(Ipv4Addr::UNSPECIFIED), mac: Default::default() }
	}
}
...
mod tests {
	...
	fn mac_constructions() {
		...
		assert_eq!(MacAddr::try_from([0x12, 0x34, 0x56, 0x78, 0x9a, 0xbc]).unwrap(), expected);
		assert_eq!(MacAddr::try_from(&[0x12, 0x34, 0x56, 0x78, 0x9a, 0xbc][..]).unwrap(), expected);
	}

	#[test]
	fn mac_to_u64() {
		assert_eq!(u64::from(MacAddr([0x12, 0x34, 0x56, 0x78, 0x9a, 0xbc])), 0x123456789abc);
	}
}
