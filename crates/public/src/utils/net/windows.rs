...
pub fn neighbor_lookup(mut dest_addr: IpAddr) -> Result<NeighborEntry> {
	...
	let dest_sockaddr = match (route.gateway, dest_addr) {
		(Some(IpAddr::V4(v4)), _) | (None, IpAddr::V4(v4)) => SOCKADDR_INET {
			Ipv4: SOCKADDR_IN {
				...
				sin_addr: IN_ADDR { S_un: IN_ADDR_0 { S_addr: u32::from(v4).to_be() } },
			},
		},
	...
	if let Err(err) = unsafe { ResolveIpNetEntry2(&mut ipnet_row, ptr::null()) } {
		return Err(Error::Windows(format!("resolve (ip -> mac address) error: {}", err)));
	}
...
pub fn get_interface_by_index_from_win32(if_index: u32) -> Result<Link> {
	...
	unsafe {
		let result =
			GetAdaptersAddresses(family, flags, ptr::null_mut(), ptr::null_mut(), &mut buffer_len);
...
fn get_interface_by_index_from_pcap(if_index: u32) -> Result<Link> {
	...
	adapters
		...
		.ok_or(Error::Windows(format!("cannot find correspond interface by if_index={}", if_index)))
}
		...
		while !adapter_ptr.is_null() {
			...
			let if_index = match adapter.Anonymous1 {
				IP_ADAPTER_ADDRESSES_LH_0 {
					Anonymous: IP_ADAPTER_ADDRESSES_LH_0_0 { Length: _, IfIndex: if_index },
