...
mod tests {
	...
	fn test_search_key_simple() {
		...
		assert_eq!(result.unwrap(), r#"{"name": "Bob", "details": {"age": 25}}"#);
	...
	fn test_extract_string_basic_functions() {
		...
		// extract string
		assert_eq!(extract_string_value_from_str(r#""hello""#).unwrap(), "hello");
		assert_eq!(extract_string_value_from_str(r#""hello","#).unwrap(), "hello");
		assert_eq!(extract_string_value_from_str(r#""hello"}"#).unwrap(), "hello");
		assert_eq!(extract_string_value_from_str(r#""""#).unwrap(), "");
		assert_eq!(extract_string_value_from_str(r#""hello world"#).unwrap(), "hello world");
		assert_eq!(
		...
		// extract array
		assert_eq!(extract_array_value_from_str("[1, 2, 3]").unwrap(), "[1, 2, 3]");
		...
		assert_eq!(extract_array_value_from_str(r#"[1, 2, 3], "other""#).unwrap(), "[1, 2, 3]");
		...
		assert_eq!(extract_primitive_value_from_str("123.45").unwrap(), "123.45");
	...
	fn test_complex_real_world_json() {
		...
		assert_eq!(search_key_case_sensitive(payload.as_bytes(), "api_version").unwrap(), "1.0");
		assert_eq!(search_key_case_sensitive(payload.as_bytes(), "status").unwrap(), "success");
