...
mod tests {
	...
	fn test_search_tag_content_complicated() {
		...
		// non-exists key
		let result = search_tag("<name>Alice</name>".as_bytes(), "age", find_bytes_ignore_case);
		assert!(result.is_err());
		assert_eq!(result.unwrap_err(), SearchError::KeyNotFound);
	}
	...
	fn test_search_tag_mixed_case() {
		...
		// case sensitive
		let result = search_tag("<name>Alice</name>".as_bytes(), "NAME", find_bytes_case_sensitive);
		assert!(result.is_err());
		assert_eq!(result.unwrap_err(), SearchError::KeyNotFound);
	...
	fn test_attribute_value_extraction_in_context() {
		...
		let result =
			search_attribute(r#"<user name='Bob'>"#.as_bytes(), "name", find_bytes_case_sensitive);
		assert_eq!(result.unwrap(), "Bob");
		...
		// Test numeric values
		let result =
			search_attribute(r#"<user age=30>"#.as_bytes(), "age", find_bytes_case_sensitive);
		assert_eq!(result.unwrap(), "30");
		...
		let result =
			search_attribute(r#"<user name="">"#.as_bytes(), "name", find_bytes_case_sensitive);
	...
	fn test_edge_cases_tag_names() {
		...
		assert_eq!(
			search_tag(payload.as_bytes(), "user_profile", find_bytes_case_sensitive).unwrap(),
			"Bob"
		);
		assert_eq!(
	...
	fn test_xml_with_full_format() {
		...
		assert_eq!(search_tag(payload.as_bytes(), "age", find_bytes_case_sensitive).unwrap(), "30");
		...
		assert_eq!(search_tag(payload1, "age", find_bytes_case_sensitive).unwrap(), "30");

		let payload2 = r#"<user id="123" name="Alice" active>"#.as_bytes();
		assert_eq!(search_attribute(payload2, "id", find_bytes_case_sensitive).unwrap(), "123");
		assert_eq!(search_attribute(payload2, "name", find_bytes_case_sensitive).unwrap(), "Alice");
		assert_eq!(search_attribute(payload2, "active", find_bytes_case_sensitive).unwrap(), "");
	}
	...
	fn test_performance_large_xml() {
		...
		let result = search_tag(large_payload.as_bytes(), "item500", find_bytes_case_sensitive);
		assert_eq!(result.unwrap(), "value500");

		let result = search_tag(large_payload.as_bytes(), "item999", find_bytes_case_sensitive);
