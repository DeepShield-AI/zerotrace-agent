...
impl<T> Buffer<T> {
	fn new(capacity: usize) -> Self {
		...
		Self { size: capacity, buffer, _marker: PhantomData }
	}
}
...
impl<T> Drop for RefCounter<T> {
	fn drop(&mut self) {
		...
		self.stats
			.max_alive
			.fetch_max(self.creation_time.elapsed().as_nanos() as u64, Ordering::Relaxed);
	}
}
...
impl<T> InnerAllocator<T> {
	fn new(capacity: usize, stats: Arc<StatsCounter>) -> Self {
		Self { allocated: 0, counter: Box::into_raw(Box::new(RefCounter::new(capacity, stats))) }
	}
...
impl RefCountable for StatsCounter {
	fn get_counters(&self) -> Vec<Counter> {
		let (new, concurrent) =
			(self.new.swap(0, Ordering::Relaxed), self.concurrent.load(Ordering::Relaxed));
		vec![
			...
			("concurrent", CounterType::Counted, CounterValue::Unsigned(concurrent)),
			(
				"concurrent_bytes",
				CounterType::Counted,
...
impl<T> Allocator<T> {
	pub fn new(capacity: usize) -> Self {
		...
		Self { capacity, inner: InnerAllocator::new(capacity, stats.clone()), stats }
	}
...
impl<T: Copy> Allocator<T> {
	...
	unsafe fn allocate_uninit(&mut self, size: usize) -> BatchedBuffer<T> {
		...
		BatchedBuffer { start: index, len: size, counter }
	}
